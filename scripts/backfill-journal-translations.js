...
for(const cycle of journal.cycles||[]){
  ...
  for(const x of cycle.daily_takeaways||[]) if(needsFix(x.title_ru,x.text_ru)) items.push({kind:'takeaway',ref:x,source:`title: ${x.title||''}\ntext: ${x.text||''}`});
}
...
let done=0;
for(let i=0;i<items.length;i+=8){const batch=items.slice(i,i+8);let out;try{out=await translate(batch)}catch(e){console.warn(`[backfill] batch ${i}-${i+batch.length} failed, skipping: ${e.message}`);continue}for(const t of out){const x=batch[Number(t.i)];if(!x)continue;const r=x.ref;if(x.kind==='read'||x.kind==='message'){if(t.topic_ru)r.topic_ru=flattenRu(t.topic_ru);if(t.ru_translation)r.ru_translation=flattenRu(t.ru_translation);if(t.simple_ru)r.simple_ru=flattenRu(t.simple_ru);if(x.kind==='read'&&t.reason_simple_ru)r.reason_simple_ru=flattenRu(t.reason_simple_ru)}else if(x.kind==='selection'){r.selection_notes_simple_ru=flattenRu(t.simple_ru||t.ru_translation||x.source)}else if(x.kind==='interest'){const topic=flattenRu(t.topic_ru||r.topic||'');const why=flattenRu(t.simple_ru||t.ru_translation||r.why||'');r.topic=topic;r.why=why;r.topic_ru=topic;r.why_ru=why;r.strength=sensible(r.strength,.55)}else if(x.kind==='loop'){const q=flattenRu(t.simple_ru||t.ru_translation||r.question||'');r.question=q;r.question_ru=q;r.priority=sensible(r.priority,.55)}else if(x.kind==='relationship'){const notes=flattenRu(t.simple_ru||t.ru_translation||r.notes||'');r.notes=notes;r.notes_ru=notes;if(t.topic_ru)r.topics=[flattenRu(t.topic_ru)];r.familiarity=sensible(r.familiarity,.25);r.trust=sensible(r.trust,.5)}else if(x.kind==='takeaway'){if(t.title_ru)r.title_ru=flattenRu(t.title_ru);if(t.text_ru)r.text_ru=flattenRu(t.text_ru)}}
  done+=batch.length;
  await save();
}
