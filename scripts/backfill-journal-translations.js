...
for(const cycle of journal.cycles||[]){
  for(const r of cycle.reads||[]) if(r.source_text && needsFix(r.ru_translation, r.simple_ru, r.reason?r.reason_ru:'нет', r.reason?r.reason_simple_ru:'нет')) items.push({kind:'read',ref:r,source:r.source_text,reason:r.reason||''});
  ...
  for(const x of cycle.daily_takeaways||[]) if(needsFix(x.title_ru,x.text_ru,x.text_simple_ru)) items.push({kind:'takeaway',ref:x,source:`title: ${x.title||''}\ntext: ${x.text||''}`});
}
...
async function translate(batch){
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    const res=await fetch(`${base}/chat/completions`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${key}`},body:JSON.stringify({model,temperature:.15,response_format:{type:'json_object'},messages:[
      {role:'system',content:`Return strict JSON {items:[{i,topic_ru,ru_translation,simple_ru,reason_ru?,reason_simple_ru?,title_ru?,text_ru?,text_simple_ru?}]}. All *_ru fields MUST be plain strings (never nested JSON objects) in Russian Cyrillic unless a proper name or code token must stay Latin. Translate meaning, not English word order. simple_ru must explain context to a smart newcomer. topic_ru is 2-6 plain Russian words. For selection/interest/loop/relationship, simple_ru should be a clear Russian explanation of the supplied text. For kind "read", if a "reason:" line is present, return reason_ru (a faithful translation of it) AND reason_simple_ru (a DIFFERENT plain-language one-sentence rewrite, not a reworded reason_ru). For kind "takeaway", the source has a "title:" and "text:" line — return title_ru (a short faithful Russian headline), text_ru (a faithful 1-2 sentence Russian translation, same technical terms), and text_simple_ru (a DIFFERENT plain-language rewrite decoding jargon for a non-technical reader — not just text_ru reworded).`},
...
let done=0;
for(let i=0;i<items.length;i+=8){const batch=items.slice(i,i+8);let out;try{out=await translate(batch)}catch(e){console.warn(`[backfill] batch ${i}-${i+batch.length} failed, skipping: ${e.message}`);continue}for(const t of out){const x=batch[Number(t.i)];if(!x)continue;const r=x.ref;if(x.kind==='read'||x.kind==='message'){if(t.topic_ru)r.topic_ru=flattenRu(t.topic_ru);if(t.ru_translation)r.ru_translation=flattenRu(t.ru_translation);if(t.simple_ru)r.simple_ru=flattenRu(t.simple_ru);if(x.kind==='read'){if(t.reason_ru)r.reason_ru=flattenRu(t.reason_ru);if(t.reason_simple_ru)r.reason_simple_ru=flattenRu(t.reason_simple_ru)}}else if(x.kind==='selection'){r.selection_notes_simple_ru=flattenRu(t.simple_ru||t.ru_translation||x.source)}else if(x.kind==='interest'){const topic=flattenRu(t.topic_ru||r.topic||'');const why=flattenRu(t.simple_ru||t.ru_translation||r.why||'');r.topic=topic;r.why=why;r.topic_ru=topic;r.why_ru=why;r.strength=sensible(r.strength,.55)}else if(x.kind==='loop'){const q=flattenRu(t.simple_ru||t.ru_translation||r.question||'');r.question=q;r.question_ru=q;r.priority=sensible(r.priority,.55)}else if(x.kind==='relationship'){const notes=flattenRu(t.simple_ru||t.ru_translation||r.notes||'');r.notes=notes;r.notes_ru=notes;if(t.topic_ru)r.topics=[flattenRu(t.topic_ru)];r.familiarity=sensible(r.familiarity,.25);r.trust=sensible(r.trust,.5)}else if(x.kind==='takeaway'){if(t.title_ru)r.title_ru=flattenRu(t.title_ru);if(t.text_ru)r.text_ru=flattenRu(t.text_ru);if(t.text_simple_ru)r.text_simple_ru=flattenRu(t.text_simple_ru)}}
  done+=batch.length;
  await save();
}
