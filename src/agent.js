...
export async function runCycle({ reason = "manual" } = {}) {
  ...
  try {
    ...
    const decision = await llm([
      ...
      { role: "user", content: `DURABLE MEMORY:\n${compact(memory, deep ? 11000 : 4500)}\nINBOX:\n${compact(incoming, deep ? 8000 : 3000)}\nPUBLIC CORPUS:\n${compact(pool, deep ? 24000 : 8000)}\nChoose at most ${maxCandidates} useful candidates (fewer is fine, silence is valid). Return candidates [${candidateFields}], inbox_translations [{id,post_id?,type,topic_ru,ru_translation,simple_ru}], ${memoryUpdateFields}daily_takeaways [{kind:"idea"|"strange"|"conversation",title,text,title_ru,text_ru,evidence_ids:[id]}] (title/text in English, never mixing in Russian words; title_ru/text_ru are the same content in plain Russian, not a literal translation of jargon), social_memory_update, selection_notes_simple_ru. ${deep ? "Also mission_report." : ""}` },
    ], { deep });
